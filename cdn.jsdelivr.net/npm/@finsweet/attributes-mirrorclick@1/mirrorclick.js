...
(() => {
  ...
  var S = (e) => (t) => `${e}${t ? `-${t}` : ""}`,
    A = (e) => {
      let t = (n, r, s) => {
        ...
        typeof b == "string"
          ? (u = b)
          : (u = b(s && "instanceIndex" in s ? s.instanceIndex : void 0));
  ...
  var U = ({ scriptAttributes: e, attributeKey: t, version: o, init: n }) => {
      var c;
      P(), (c = window.fsAttributes)[t] || (c[t] = {});
      let { preventsLoad: r, attributes: s } = g(e),
    ...
    k = () => {
      let e = x("src", "finsweet", { operator: "contains" }),
        t = x("dev");
      return [...document.querySelectorAll(`script${e}, script${t}`)].reduce(
        (r, s) => {
          var c;
          let i =
            s.getAttribute(p.dev.key) ||
  ...
  var C = `fs-${l}`,
    ...
    D = "delay",
    y = {
      element: { key: `${C}-element`, values: { trigger: O, target: S($) } },
      delay: { key: `${C}-${D}` },
    },
    [I, ft] = A(y);
  var {
    ...
    K = () => {
      let e = E(window, "click", ({ target: t }) => {
        if (!(t instanceof Element)) return;
        let o = t.closest(I("element", "trigger", { operator: "prefixed" }));
        if (!o) return;
        let n = B(o, F),
          r = document.querySelectorAll(
            I("element", "target", { instanceIndex: n })
          );
        for (let s of r) {
  ...
  U({ init: K, version: v, attributeKey: l });
})();
